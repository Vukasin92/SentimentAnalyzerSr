...
// This work is done as part of master thesis of Nikola Milosevic on University
...
$stopWordsStr = file_get_contents("StopWordsListEnrichedMedium-stemmed.txt");
$stopWords = explode(" ",$stopWordsStr);

function stem($str){
global $dictionary, $rules, $stopWords;
$outtext = "";
...
if (isset($str))
{
	...
	$substr1="";
	$substr2="";
	//Tokenizes and set interpucntion marks to be separated with blank char form word
	for($i=0;$i<strlen($text);$i++)
	{
		if(($text[$i]=="." or $text[$i]=="," or $text[$i]=="!" or $text[$i]==":" or $text[$i]=="?" or $text[$i]=="(" or $text[$i]==")" or $text[$i]==";") and $text[$i-1]!=" ")
		{
	...
	{
		$currtoken = $tokens[$i];
		//check if token is biti, change to biti if true
		...
		if (!in_array($tokens[$i], $stopWords)) {
			$outtext=$outtext . " " . $tokens[$i];
		}
	}
